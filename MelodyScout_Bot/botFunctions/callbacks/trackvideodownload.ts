...
export class TrackvideodownloadCallback {
  ...
  async run (ctx: CallbackQueryContext<Context>): Promise<void> {
    ...
    if (!youtubeTrackInfo.success) return await this.ctxFunctions.answerCallbackQuery(ctx, '⚠ - Ocorreu um erro ao tentar obter a URL de download do vídeo')
    await ctx.answerCallbackQuery('🎥 - Enviando vídeo...')
    this.ctxFunctions.replyWithVideo(ctx, new InputFile({ url: youtubeTrackInfo.videoWithAudioUrl }), {
      caption: `<b>${track}</b> de <b>${artist}</b>\n\nSolicitado por: <b><a href='tg://user?id=${ctx.from.id}'>${ctx.from.first_name}</a></b>`,
      reply_to_message_id: messageId
    }).catch(async (err) => {
      await this.ctxFunctions.answerCallbackQuery(ctx, '⚠ - Ocorreu um erro ao tentar enviar o vídeo')
      console.log(err)
    })

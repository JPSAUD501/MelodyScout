...
export class TracklistCommand {
  ...
  async run (ctx: CommandContext<Context>): Promise<void> {
    ...
    if (trackerChatDBResponse.trackingUsers.length === 0) {
      await this.ctxFunctions.reply(ctx, 'Parece que ninguém está sendo rastreado nesse grupo! Para começar a rastrear um perfil, envie o comando /track junto com o nome de usuário do Last.fm como no exemplo a seguir: <code>/track MelodyScout</code>')
      return
    }

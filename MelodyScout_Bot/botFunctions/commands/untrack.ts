...
export class UntrackCommand {
  ...
  async run (ctx: CommandContext<Context>): Promise<void> {
    ...
    if (message.length < 2) {
      await this.ctxFunctions.reply(ctx, 'Ops! Parece que você não me enviou o nome de usuário do perfil que você quer parar de rastrear! Utilize o comando /untrack junto com o nome de usuário do Last.fm como no exemplo a seguir: <code>/untrack MelodyScout</code>')
      return
    }
    ...
    if (trackerChatDBResponse.trackingUsers.find((user) => user === username) == null) {
      await this.ctxFunctions.reply(ctx, 'Parece que esse perfil já não está sendo rastreado nesse grupo! Por favor, verifique se você digitou o nome de usuário corretamente e tente novamente! Caso acredite que esse é algum erro, por favor, entre em contato com o desenvolvedor do bot utilizando o comando /contact!')
      return
    }
    ...
    if (!removedUserFromTrackInChat.success) {
      await this.ctxFunctions.reply(ctx, 'Que ódio! Parece que eu não consegui remover esse perfil da lista de perfis que estão sendo rastreados nesse grupo! Por favor, tente novamente mais tarde ou entre em contato com o desenvolvedor do bot utilizando o comando /contact!')
      return
    }

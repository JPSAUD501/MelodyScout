...
export class MyuserCommand {
  ...
  async run (ctx: CommandContext<Context>): Promise<void> {
    ...
    if (!telegramUserDBResponse.success) return await this.ctxFunctions.reply(ctx, 'Ops! Parece que eu não consegui verificar se você já está cadastrado no MelodyScout! Por favor, tente novamente mais tarde ou entre em contato com o desenvolvedor do bot utilizando o comando /contact!')
    if (telegramUserDBResponse.lastfmUser !== null) return await this.ctxFunctions.reply(ctx, 'Ops! Parece que você já está cadastrado no MelodyScout! Por favor, utilize o comando /forgetme para que eu esqueça o seu cadastro no MelodyScout e depois utilize o comando /myuser novamente para cadastrar o seu nome de usuário do Last.fm!')
    const message = ((ctx.message?.text?.split(' ')) != null) ? ctx.message?.text?.split(' ') : []
    if (message.length < 2) return await this.ctxFunctions.reply(ctx, 'Ops! Parece que você não me informou o seu nome de usuário do Last.fm! Por favor, tente novamente informando o seu nome de usuário do Last.fm como no exemplo a seguir: <code>/myuser MelodyScout</code>')
    const username = message[1]
    const userExists = await this.msLastfmApi.checkIfUserExists(username)
    if (!userExists.success) return await this.ctxFunctions.reply(ctx, 'Ops! Parece que eu não consegui verificar se o seu nome de usuário do Last.fm existe! Por favor, tente novamente mais tarde ou entre em contato com o desenvolvedor do bot utilizando o comando /contact!')
    if (!userExists.exists) return await this.ctxFunctions.reply(ctx, 'Ops! Parece que o nome de usuário do Last.fm que você me informou não existe! Por favor, tente novamente informando um nome de usuário do Last.fm válido!')
    const updateTelegramUserDBResponse = await this.prismaDB.update.telegramUser(telegramUserId, username)
    if (!updateTelegramUserDBResponse.success) return await this.ctxFunctions.reply(ctx, 'Ops! Parece que eu não consegui registrar o seu nome de usuário do Last.fm! Por favor, tente novamente mais tarde ou entre em contato com o desenvolvedor do bot utilizando o comando /contact!')

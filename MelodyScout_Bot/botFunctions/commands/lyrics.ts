...
export class LyricsCommand {
  ...
  async run (ctx: CommandContext<Context>): Promise<void> {
    ...
    if (!albumInfo.success) {
      await this.ctxFunctions.reply(ctx, 'Não foi possível resgatar as informações do álbum dessa música, tente novamente mais tarde! Se o problema persistir entre em contato com o meu desenvolvedor utilizando o comando /contact')
      return
    }
    ...
    if (trackLyrics === null) {
      await this.ctxFunctions.reply(ctx, 'Não foi possível resgatar a letra dessa música, tente novamente mais tarde! Se o problema persistir entre em contato com o meu desenvolvedor utilizando o comando /contact')
      return
    }

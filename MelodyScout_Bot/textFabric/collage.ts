...
export function getCollageText (ctxLang: string | undefined, userInfo: UserInfo, collageUrl: string, topTracks: CollageTrackData[], period: 'overall' | '7day' | '1month' | '3month' | '6month' | '12month'): string {
  ...
  switch (true) {
    case (period === 'overall'): {
      textArray.push(lang(ctxLang, { key: 'tfCollageTopTracksOverallHeader', value: '<b>[🎵] Músicas mais ouvidas desde sempre:</b>' }))
      break
    }
    case (period === '7day'): {
      textArray.push(lang(ctxLang, { key: 'tfCollageTopTracks7dayHeader', value: '<b>[🎵] Músicas mais ouvidas nos últimos 7 dias:</b>' }))
      break
    }
    case (period === '1month'): {
      textArray.push(lang(ctxLang, { key: 'tfCollageTopTracks1monthHeader', value: '<b>[🎵] Músicas mais ouvidas no último mês:</b>' }))
      break
    }
    case (period === '3month'): {
      textArray.push(lang(ctxLang, { key: 'tfCollageTopTracks3monthHeader', value: '<b>[🎵] Músicas mais ouvidas nos últimos 3 meses:</b>' }))
      break
    }
    case (period === '6month'): {
      textArray.push(lang(ctxLang, { key: 'tfCollageTopTracks6monthHeader', value: '<b>[🎵] Músicas mais ouvidas nos últimos 6 meses:</b>' }))
      break
    }
    case (period === '12month'): {
      textArray.push(lang(ctxLang, { key: 'tfCollageTopTracks12monthHeader', value: '<b>[🎵] Músicas mais ouvidas no último ano:</b>' }))
      break
    }
    default: {

...
export function getHistoryText (userInfo: UserInfo, userRecentTracks: UserRecentTracks): string {
  ...
  textArray.push(`<b><a href="${recenttracks.track[0].image[recenttracks.track[0].image.length - 1]['#text']}">️️</a><a href="${config.melodyScout.userImgUrl}">️️</a>Histórico de reprodução de <a href="${urlLimiter(user.url)}">${user.realname.length > 0 ? sanitizeText(user.realname) : sanitizeText(user.name)}</a></b>`)
  textArray.push('')
  if (recenttracks.track[0]['@attr']?.nowplaying === 'true') {
    const track = recenttracks.track[0]
    textArray.push(`<b>[🎧] Ouvindo agora <a href="${urlLimiter(track.url)}">${sanitizeText(track.name)} de ${track.artist.name}</a></b>`)
    textArray.push('')
  }
  ...
  if (recenttracks.track.length > 0) {
    for (let i = 0; i < recenttracks.track.length; i++) {
      ...
      textArray.push(`- <a href="${urlLimiter(track.url)}"><b>${sanitizeText(track.name)}</b> de <b>${track.artist.name}</b></a>`)
    }
    textArray.push('')
  }

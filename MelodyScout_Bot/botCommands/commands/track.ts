...
export class TrackCommand {
  ...
  async run (ctx: CommandContext<Context>): Promise<void> {
    ...
    if (message.length < 2) return await this.ctxFunctions.reply(ctx, 'Ops! Parece que você não me enviou o nome de usuário do perfil que você deseja que eu rastreie! Por favor, envie o comando /track junto com o nome de usuário do Last.fm como no exemplo a seguir: <code>/track MelodyScout</code>')
    ...
    if (trackerChatDBResponse.trackingUsers.find((user) => user === username) != null) return await this.ctxFunctions.reply(ctx, 'Parece que esse perfil já está sendo rastreado nesse grupo! Por favor, verifique se você digitou o nome de usuário corretamente e tente novamente! Caso acredite que esse é algum erro, por favor, entre em contato com o desenvolvedor do bot utilizando o comando /contact!')
    const userExists = await this.msLastfmApi.checkIfUserExists(username)
    if (!userExists.success) return await this.ctxFunctions.reply(ctx, 'Sinto muito, infelizmente eu não consegui verificar se o perfil que você me enviou existe ou não! Por favor, tente novamente mais tarde! Caso esse erro persista, por favor, entre em contato com o desenvolvedor do bot utilizando o comando /contact!')
    if (!userExists.exists) return await this.ctxFunctions.reply(ctx, 'Ops! Parece que o perfil que você me enviou não existe ou não é valido! Por favor, verifique se você digitou o nome de usuário corretamente e tente novamente! Caso acredite que esse é algum erro, por favor, entre em contato com o desenvolvedor do bot utilizando o comando /contact!')
    const userAboutMe = await this.msLastfmApi.getUserAboutMe(username)
    if (!userAboutMe.success) return await this.ctxFunctions.reply(ctx, 'Que raiva, infelizmente eu não consegui verificar se o perfil que você me enviou me autorizou a rastreá-lo! Por favor, tente novamente mais tarde! Caso esse erro persista, por favor, entre em contato com o desenvolvedor do bot utilizando o comando /contact!')
    const lastfmUserAllowedTrackingChatIds = getUserAllowedTrackingChatIds(userAboutMe.aboutMe).includes(`${ctx.chat.id}`)
    if (!lastfmUserAllowedTrackingChatIds) return await this.ctxFunctions.reply(ctx, `Pela minha politica de privacidade eu preciso ter certeza que você é o dono do perfil ou que o dono do mesmo te autorizou a rastrear ele! Para isso eu preciso que seja adicionado na seção "<code>about me</code>" do perfil no Last.fm o seguinte texto: "<code>T:${ctx.chat.id}</code>"!\n\nApós isso me envie novamente o comando comando /track junto com o nome de usuário do Last.fm como no exemplo a seguir: <code>/track MelodyScout</code>\n\nCaso acredite que esse é algum erro, por favor, entre em contato com o desenvolvedor do bot utilizando o comando /contact!`)
    const newUserToTrackInChatResult = await this.prismaDB.update.trackerChat(ctx.chat.id.toString(), [username], 'add')
    if (!newUserToTrackInChatResult.success) return await this.ctxFunctions.reply(ctx, 'M*rda! Parece que eu não consegui adicionar esse perfil na lista de perfis que estão sendo rastreados nesse grupo! Por favor, tente novamente mais tarde ou entre em contato com o desenvolvedor do bot utilizando o comando /contact!')

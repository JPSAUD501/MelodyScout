...
export class TrackCommand {
  ...
  async run (ctx: CommandContext<Context>): Promise<void> {
    ...
    if (trackerChatDBResponse.trackingUsers.find((user) => user === username)) return await this.ctxFunctions.ctxReply(ctx, 'Parece que esse perfil já está sendo rastreado nesse grupo! Por favor, verifique se você digitou o nome de usuário corretamente e tente novamente! Caso acredite que esse é algum erro, por favor, entre em contato com o desenvolvedor do bot utilizando o comando /contact!');

...
export class TracklistCommand {
  ...
  async run (ctx: CommandContext<Context>): Promise<void> {
    ...
    await this.ctxFunctions.ctxReply(ctx, 'Estou verificando a lista de perfis que estão sendo rastreados nesse grupo...')
    ...
    if (trackerChatDBResponse.trackingUsers.length === 0) return await this.ctxFunctions.ctxReply(ctx, 'Parece que ninguém está sendo rastreado nesse grupo! Para começar a rastrear um perfil, envie o comando /track junto com o nome de usuário do Last.fm como no exemplo a seguir: <code>/track MelodyScout</code>')
    await this.ctxFunctions.ctxReply(ctx, `Aqui está a lista de perfis que estão sendo rastreados nesse grupo: <code>${trackerChatDBResponse.trackingUsers.join(', ')}</code>`)
  }
}

...
import { zodLyricsApiError } from '../types/errors/zodLyricsApiError'
...
export const msApiFetch = async (url: string, method: string | undefined, headers: object | undefined, data: object | undefined, expectedZod: ZodObject<any> | ZodAny | ZodString | ZodUnion<any>): Promise<MsApiFetchResponse> => {
  try {
    ...
    const lyricsApiError = zodLyricsApiError.safeParse(jsonResponse)
    if (lyricsApiError.success) {
      return {
        success: false,
        errorType: 'lyricsApiError',
        errorData: lyricsApiError.data
      }
    }
